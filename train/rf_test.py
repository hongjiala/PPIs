...
for model_trained in models:
    ...
    for i in range(10):
        ...
        rfr = dill.load(open('rfr_model/%s%d.obj'%(model_trained,i), 'rb'))



        test=np.load('data_new/%s_test.npz'%model_trained)
        human_test,virus_test,y_tes=test['human'],test['virus'],test['label']
        human_test=np.reshape(human_test,(human_test.shape[0],human_test.shape[1]))
        virus_test = np.reshape(virus_test,(virus_test.shape[0],virus_test.shape[1]))
